...
// Generate Voucher No (e.g., PV-2024-001)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($date) || empty($head) || empty($payee) || empty($amount)) {
        ...
        try {
            if ($id) {
                ...
                $stmt->execute([$date, $head, $payee, $amount, $method, $ref, $narration, $_POST['bank_name']??null, $_POST['cheque_no']??null, ($_POST['cheque_date']??null)?:null, $id, $type]);
                ...
                $stmt->execute([$vNo, $type, $date, $head, $payee, $amount, $method, $ref, $narration, $_POST['bank_name']??null, $_POST['cheque_no']??null, ($_POST['cheque_date']??null)?:null]);
                $successMsg = "Payment Voucher generated successfully!";
            }
        } catch (PDOException $e) {
...
// Search and Filter
$search = $_GET['search'] ?? '';
$query = "SELECT * FROM vouchers WHERE voucher_type = 'Payment'";
$params = [];
...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-slate-800">Payment Vouchers</h1>
        </div>
    ...
    <form class="bg-white p-4 rounded-none border border-slate-100 shadow-sm flex gap-4 items-end">
        <div class="flex-1">
            ...
            <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search voucher no, payee..." class="w-full bg-slate-50 border border-slate-200 rounded-none px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500">
        </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50">
                    <?php if (empty($vouchers)): ?>
                        <tr><td colspan="7" class="px-6 py-12 text-center text-slate-400">No payment vouchers found.</td></tr>
                    <?php else: ?>
                        <?php foreach ($vouchers as $v): ?>
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-6 py-4 text-sm font-bold text-indigo-600"><?php echo htmlspecialchars($v['voucher_no']); ?></td>
...
<div id="voucherModal" class="hidden fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
    <div class="bg-white rounded-none shadow-2xl w-full max-w-xl overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex justify-between items-center">
            <h2 id="modalTitle" class="text-xl font-bold text-slate-800">New Payment Voucher</h2>
        ...
        <form method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Payee (Paid To) *</label>
                    <input type="text" name="payee_payer" id="payee_payer" required placeholder="Enter name of recipient" class="w-full bg-slate-50 border border-slate-200 rounded-none px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="narration" id="voucher_narration" rows="2" placeholder="Being paid for..." class="w-full bg-slate-50 border border-slate-200 rounded-none px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500"></textarea>
                </div>
            </div>
            <div class="flex gap-3 pt-4">
                ...
                <button type="submit" class="flex-1 px-6 py-3 bg-indigo-600 text-white rounded-none font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-100">Generate Voucher</button>
            </div>
        </form>
    </div>
</div>
...
function openModal() {
    ...
    document.getElementById('modalTitle').innerText = 'New Payment Voucher';
...
function editVoucher(data) {
    openModal();
    document.getElementById('modalTitle').innerText = 'Edit Payment Voucher';
